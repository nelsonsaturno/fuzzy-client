 ...
 */
public class CreateFuzzyDomainFromTest {
    ...
    public CreateFuzzyDomainFromTest() {
    }
    ...
    public void createAnOperation() throws Exception{
        TranslationResult translate = connector.translate("CREATE FUZZY DOMAIN names AS VALUES FROM fuzzy_ddl_test.people.name");
        assertNull(translate.sql);
        assertNotNull(translate.operations);
        assertEquals(1, translate.operations.size());
        assertEquals(CreateFuzzyDomainFromColumnOperation.class, translate.operations.get(0).getClass());
        CreateFuzzyDomainFromColumnOperation o = (CreateFuzzyDomainFromColumnOperation) translate.operations.get(0);
        assertEquals("names", o.getDomainName());
        assertEquals("fuzzy_ddl_test", o.getSchemaName());
        assertEquals("people", o.getTableName());
        assertEquals("name", o.getColumnName());
    }
}
